<template>
  <div class="p-0 h-100">
    <ul class="MuiList-root MuiList-padding" style="width: 100%; margin-top: 0; padding: 3px 0 0; background-color: white;">
      <a class="makeLinkNormal" href="javascript:void(0)">
        <div class="MuiButtonBase-root MuiListItem-root pt-1 MuiListItem-button MuiListItem-alignItemsFlexStart" tabindex="0" role="button" aria-disabled="false" style="margin-left: 0px; padding-right: 0px; padding-left: 15px; padding-bottom: 4px;">
          ...
          <router-link :to="{name: 'ContactUs'}" class="nav-item nav-link">
    ...
    <div class="row text-center justify-content-between m-0 w-100" :dir="$parent.language === 'en' ? 'ltr' : 'rtl'">
      <div v-for="category in filteredCategories" :style="{'height': '164px',  'width': '45%', 'border-radius': '7px', 'margin-bottom': '3px', 'background': 'url('+category.image+') center center / cover no-repeat', 'background-size': '200px', 'border': '1px solid white', 'min-height': '240px'}">
        <router-link :to="{name: 'CategoryDetails', params: { id: category.id }}" class="nav-item nav-link">
          ...
          <p style="top: -135px; left: 0px; position: relative; z-index: 3; display: block; color: white; font-size: 1.4rem;">
            {{ $parent.language === 'ar' ? category.category_ar : category.category }}
          </p>
        </router-link>
      </div>

    </div>
    <div v-if="$parent.language === 'en'" class="free-space-115">Powered by <a href="www.tryryda.com">TryRyda</a></div>
    ...
    <div v-if="!$parent.quantity > 0" class="action-button-english" style="background-color: white; padding-bottom: 8px; margin-bottom: 0px; height: 60px; z-index: 4;">
      <button v-on:click="$router.push('/search')" class="MuiButtonBase-root MuiButton-root MuiButton-contained mb-1  ml-1 mx-auto MuiButton-containedPrimary" tabindex="0" type="button" :dir="$parent.language === 'en' ? 'ltr' : 'rtl'" :style="{'width': '97%', 'height': '100%', 'box-shadow': 'none', 'text-transform': 'none', 'background': $parent.settings.button_color}">
        <span class="MuiButton-label">
          ...
          <span v-else style="font-size: 1rem;">بدء الطلب</span>
